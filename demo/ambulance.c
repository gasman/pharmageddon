...
static void blitfill_tex(uint32_t *pixels, double *zbuffer, int offset, int len, double z1, double z2, double bright1, double bright2, int u1, int u2, int v1, int v2) {
    uint32_t *pixels_pos = pixels + offset;
    double *zbuffer_pos = zbuffer + offset;
    double z = z1;
    double zstep = (z2 - z1) / (double)len;
    ...
    for (int i = 0; i < len; i++) {
        if (z < *zbuffer_pos) {
            unsigned char *texel = ambulance_texture.data + ((v>>16)*ambulance_texture.width + (u>>16)) * 4;
            *pixels_pos = (
                (texel[0] << 24)
                | (texel[1] << 16)
                | (texel[2] << 8)
...
static void gfx3d_gouraud_tex_tri(uint32_t *pixels, double *zbuffer, vertex_out_attrs va1, vertex_out_attrs va2, vertex_out_attrs va3) {
    vec3 v1 = va1.position;
    vec3 v2 = va2.position;
    vec3 v3 = va3.position;
    /* find z component of cross product to test for back-face culling */
    ...
    if (xstep1 < xstep2) {
        for (ry=y1; ry<=y3; ry++) {
            ...
            if ((ry >= 0) && (ry < 192) && (startx < 192) && (endx >= 0)) {
                ...
                if (endx > 192 - 1) endx = 192 - 1;
                if ((endx - startx + 1) > 0) {
                    blitfill_tex(pixels, zbuffer, startx + py, endx - startx + 1, dz1, dz2, db1, db2, du1, du2, dv1, dv2);
                }
            }
            dx1 += xstep1;
            dz1 += zstep1;
